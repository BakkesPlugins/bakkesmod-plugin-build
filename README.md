...
| `plugin-outdir` | Output directory for the plugin build. Can be an absolute path, a relative path (relative to solution directory), or use the `$(SolutionDir)` MSBuild variable. This value is passed to MSBuild as `/p:OutDir=` to override the project settings. | No | `plugins\` |
